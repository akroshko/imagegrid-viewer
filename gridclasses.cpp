...
bool ImageGrid::load_grid(GridSetup *grid_setup) {
  ...
  for (INT_T i = 0; i < this->grid_image_size->wimage(); i++) {
    ...
    for (INT_T j = 0; j < this->grid_image_size->himage(); j++) {
      ...
      squares[i][j].load_file(grid_setup->filenames[ij]);
    }
  }
  return successful;
}
...
bool TextureGrid::load_texture (TextureGridSquare &dest_square,
  ...
  if (dest_square.texture_array[zoom_level]->display_texture) {
      auto lock_surface_return=SDL_LockSurface(dest_square.texture_array[zoom_level]->display_texture);
...
void TextureGrid::update_textures (ImageGrid *grid,
  ...
  for (INT_T j = 0; j < this->grid_image_size->himage(); j++) {
    ...
    for (INT_T i = 0; i < this->grid_image_size->wimage(); i++) {
      std::unique_lock<std::mutex> lock(this->squares[i][j].texture_array[zoom_level]->display_mutex, std::defer_lock);
      ...
      if (lock.try_lock()) {
        if (!load_all &&
          ...
          if (this->squares[i][j].texture_array[zoom_level]->display_texture == nullptr) {
            load_texture(this->squares[i][j], grid->squares[i][j], zoom_level);
          }
        }
      }
    }
